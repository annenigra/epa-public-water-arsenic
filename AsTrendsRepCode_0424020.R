...
library(openxlsx)
write.xlsx(cws, file="CWSAsSYR3.xlsx") ;write.xlsx(cws, file="~/Google Drive/Research/EPAAsTrajectories/March2020/CWSAsSYR3.xlsx")
write.xlsx(county, file="CountyAsSYR3.xlsx") ;write.xlsx(county, file="~/Google Drive/Research/EPAAsTrajectories/March2020/CountyAsSYR3.xlsx")
...
####### 1. Table 1. Distribution of 3-year CWS averages, stratified by pop served, water type, region, & cluster #########
cws$ThreeYearAs20062008<-as.numeric(cws$ThreeYearAs20062008)
cws$ThreeYearAs20092011<-as.numeric(cws$ThreeYearAs20092011)
...
gwcws <-round(gwcws[,],2)
gwcws<-rbind(gwcws,mds)
rownames(gwcws)[4]<-"Mean difference (95% CI)"
gwcws <- rbind(gwcws, Type= c("GW","GW"))
...
Table1A<-cbind(Column1 = "NAME", Table1A)

# Transpose into better format
Table1 <- data.frame(t(Table1A[-1]))
colnames(Table1) <- Table1A[, 1]
colnames(Table1)[1]<-"N";colnames(Table1)[2]<-"% at/above MCL";colnames(Table1)[3]<-"Arithmetic mean";colnames(Table1)[4]<-"Mean difference"



write.xlsx(Table1,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Table1 .xlsx")

### end 



#######       1B. Supplemental Table 15: adjust stratified models by source water type  ######
...
write.xlsx(Table1B,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 15.xlsx")
...
### end 
#######       1C. Add column to Table 1 and Supp Table 15: OR of MCL exceedance ######
...
write.xlsx(Table1Add,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Table1 .xlsx")
...
oratsadjtobind<-rbind("ref",allr,"ref",pop1r,"ref",pop2r,"ref",pop3r,"ref",pop4r,"ref",
                   pop5r,"ref",akhir,"ref",cmwr,"ref",emwr,"ref",mar,"ref",ner,"ref",pnwr,"ref",ser,"ref",swr,"ref",c1r,"ref",c2r,"ref",c3r,"ref",c4r,"ref",c5r,"ref",c6r,"ref",c7r,"ref",c8r)
write.xlsx(oratsadj,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/oratsadj.xlsx")

# Add these to Table 1
STable15Add<-cbind(Table1B,oratsadjtobind)
write.xlsx(STable15Add,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 15.xlsx")
...
#######       1D. Extra Table. OR of MCL exceedance (GEE, both 3-yr) comparing subgroup membership #######
# Create dummy vars
...
write.xlsx(ExtraTable,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Extra Table ORs.xlsx")


### Plot in forestplot
OR_data <- data.frame(read.xlsx("~/Google Drive/Research/EPAAsTrajectories/March2020/Odds_Ratio_Figure_data.xlsx"))

# This order is: All, SE, MA,CMW,EMW, NE, PNW, SW
OR_data$Condition <- factor(OR_data$Condition, levels = c("Source water type", "Size of population served","Region","County-cluster"))
...
raincloud_theme = theme(
  ...
  axis.text.x = element_text(angle = 70, vjust=0.85,hjust = 0.85),
...
pdf(file=paste0( folder.output , "/Extra Figure.pdf"), 23, 12)
...
p <- ggplot(data=OR_data,
  ...
  xlab('Subgroup')+ ylab("Odds ratio (95% Confidence Interval) of MCL exceedance\nduring either time period (2006-2008 or 2009-2011)\ncomparing subgroup membership versus non-membership")+
  ...
  theme_bw() +
  raincloud_theme+
  theme(strip.text = element_text(size=14,face="bold"),strip.background = element_rect(color="#525252", fill="white", size=2, linetype="solid"))+
  scale_y_log10(breaks=c(0.2,0.3,0.4,0.5,1,2,3,4,5))
p
dev.off()
...
####### 2. Table 2. Characteristics of CWSs by compliance categories ####### 
...
cwsall<-rbind(n,npgw,npsw,pops,npAKHI,npCMW,npEMW,npMA,npNE,npPNW,npSE,npSW,npTribal,state1,state2,state3)
...
cat1<-rbind(n,npgw,npsw,pops,npAKHI,npCMW,npEMW,npMA,npNE,npPNW,npSE,npSW,npTribal,state1,state2,state3)
...
cat3<-rbind(n,npgw,npsw,pops,npAKHI,npCMW,npEMW,npMA,npNE,npPNW,npSE,npSW,npTribal,state1,state2,state3)
...
cat2<-rbind(n,npgw,npsw,pops,npAKHI,npCMW,npEMW,npMA,npNE,npPNW,npSE,npSW,npTribal,state1,state2,state3)
...
cat4<-rbind(n,npgw,npsw,pops,npAKHI,npCMW,npEMW,npMA,npNE,npPNW,npSE,npSW,npTribal,state1,state2,state3)
cat4 <- rbind(cat4, Cateogry= c("Category 4, High/High"))

Table2<-cbind(cwsall,cat1,cat2,cat3,cat4)
write.xlsx(Table2,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Table 2.xlsx")
...
####### 3. Supplemental Table 1 and Supplemental Figure 2. Description of yearly averages for each CWS  ########
...
#pdf(file=paste0( folder.output , "/ Supp Figure 2 missingness pattern.pdf"), 30, 20)
...
write.xlsx(ST1dist,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 1A Yearly CWS Distributions.xlsx")
...
####### 4. Supplemental Table 1B. Sensitivity analysis: complete case analysis at CWS level ######
...
write.xlsx(ST1Bdist,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 1B Complete Case.xlsx")


### end


####### 5. Supplemental Table 2. Distribution of county yearly averages  ###########
# County-level data is in notlowcounties50subset
###
notlowcounties50subset<-merge(notlowcounties50subset,clusters,by="CountyFIPS",all.x=T);dim(notlowcounties50subset)# 

# This final data is in notlowcounties50subset
describe(notlowcounties50subset$CountyFIPS)
describe(notlowcounties50subset$WeightedAs2006)

## Supplemental Table 1 ##
# Distributions by year
# 2006
nav<-nrow(notlowcounties50subset[which(!is.na(notlowcounties50subset$WeightedAs2006)),])
nmiss<-nrow(notlowcounties50subset[which(is.na(notlowcounties50subset$WeightedAs2006)),])
quant<-as.data.frame(quantile(notlowcounties50subset$WeightedAs2006,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2006<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2006>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(notlowcounties50subset$WeightedAs2006),na.rm=T))
mean<-mean(notlowcounties50subset$WeightedAs2006,na.rm=T)
colnames(quant)[1] <- "2006"
WeightedAs2006<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(WeightedAs2006)[1] <- "N";rownames(WeightedAs2006)[2] <- "N missing";rownames(WeightedAs2006)[12] <- "% below MRL"
rownames(WeightedAs2006)[13] <- "% above MCL";rownames(WeightedAs2006)[14] <- "Geometric mean";rownames(WeightedAs2006)[15] <- "Arithmetic mean"
# 2007
nav<-nrow(notlowcounties50subset[which(!is.na(notlowcounties50subset$WeightedAs2007)),])
nmiss<-nrow(notlowcounties50subset[which(is.na(notlowcounties50subset$WeightedAs2007)),])
quant<-as.data.frame(quantile(notlowcounties50subset$WeightedAs2007,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2007<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2007>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(notlowcounties50subset$WeightedAs2007),na.rm=T))
mean<-mean(notlowcounties50subset$WeightedAs2007,na.rm=T)
colnames(quant)[1] <- "2007"
WeightedAs2007<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(WeightedAs2007)[1] <- "N";rownames(WeightedAs2007)[2] <- "N missing";rownames(WeightedAs2007)[12] <- "% below MRL"
rownames(WeightedAs2007)[13] <- "% above MCL";rownames(WeightedAs2007)[14] <- "Geometric mean";rownames(WeightedAs2007)[15] <- "Arithmetic mean"
# 2008
nav<-nrow(notlowcounties50subset[which(!is.na(notlowcounties50subset$WeightedAs2008)),])
nmiss<-nrow(notlowcounties50subset[which(is.na(notlowcounties50subset$WeightedAs2008)),])
quant<-as.data.frame(quantile(notlowcounties50subset$WeightedAs2008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(notlowcounties50subset$WeightedAs2008),na.rm=T))
mean<-mean(notlowcounties50subset$WeightedAs2008,na.rm=T)
colnames(quant)[1] <- "2008"
WeightedAs2008<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(WeightedAs2008)[1] <- "N";rownames(WeightedAs2008)[2] <- "N missing";rownames(WeightedAs2008)[12] <- "% below MRL"
rownames(WeightedAs2008)[13] <- "% above MCL";rownames(WeightedAs2008)[14] <- "Geometric mean";rownames(WeightedAs2008)[15] <- "Arithmetic mean"
# 2009
nav<-nrow(notlowcounties50subset[which(!is.na(notlowcounties50subset$WeightedAs2009)),])
nmiss<-nrow(notlowcounties50subset[which(is.na(notlowcounties50subset$WeightedAs2009)),])
quant<-as.data.frame(quantile(notlowcounties50subset$WeightedAs2009,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2009<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2009>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(notlowcounties50subset$WeightedAs2009),na.rm=T))
mean<-mean(notlowcounties50subset$WeightedAs2009,na.rm=T)
colnames(quant)[1] <- "2009"
WeightedAs2009<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(WeightedAs2009)[1] <- "N";rownames(WeightedAs2009)[2] <- "N missing";rownames(WeightedAs2009)[12] <- "% below MRL"
rownames(WeightedAs2009)[13] <- "% above MCL";rownames(WeightedAs2009)[14] <- "Geometric mean";rownames(WeightedAs2009)[15] <- "Arithmetic mean"
# 2010
nav<-nrow(notlowcounties50subset[which(!is.na(notlowcounties50subset$WeightedAs2010)),])
nmiss<-nrow(notlowcounties50subset[which(is.na(notlowcounties50subset$WeightedAs2010)),])
quant<-as.data.frame(quantile(notlowcounties50subset$WeightedAs2010,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2010<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2010>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(notlowcounties50subset$WeightedAs2010),na.rm=T))
mean<-mean(notlowcounties50subset$WeightedAs2010,na.rm=T)
colnames(quant)[1] <- "2010"
WeightedAs2010<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(WeightedAs2010)[1] <- "N";rownames(WeightedAs2010)[2] <- "N missing";rownames(WeightedAs2010)[12] <- "% below MRL"
rownames(WeightedAs2010)[13] <- "% above MCL";rownames(WeightedAs2010)[14] <- "Geometric mean";rownames(WeightedAs2010)[15] <- "Arithmetic mean"
# 2011
nav<-nrow(notlowcounties50subset[which(!is.na(notlowcounties50subset$WeightedAs2011)),])
nmiss<-nrow(notlowcounties50subset[which(is.na(notlowcounties50subset$WeightedAs2011)),])
quant<-as.data.frame(quantile(notlowcounties50subset$WeightedAs2011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(notlowcounties50subset[which(notlowcounties50subset$WeightedAs2011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(notlowcounties50subset$WeightedAs2011),na.rm=T))
mean<-mean(notlowcounties50subset$WeightedAs2011,na.rm=T)
colnames(quant)[1] <- "2011"
WeightedAs2011<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(WeightedAs2011)[1] <- "N";rownames(WeightedAs2011)[2] <- "N missing";rownames(WeightedAs2011)[12] <- "% below MRL"
rownames(WeightedAs2011)[13] <- "% above MCL";rownames(WeightedAs2011)[14] <- "Geometric mean";rownames(WeightedAs2011)[15] <- "Arithmetic mean"


# Mean difference models
# new dataset in long format
library(tidyr)
is.factor(notlowcounties50subset$PWSID)
notlowcounties50subset$PWSID<-as.factor(notlowcounties50subset$PWSID)
finallong <- gather(notlowcounties50subset, year, arsenic, WeightedAs2006:WeightedAs2011, factor_key=TRUE)
finallong
md2006<-"0 (reference)"
fit1 <- glm(arsenic ~ year, family=gaussian(link="identity"), data=finallong)
summary(fit1)
...
colnames(mds)[1]<-"2006";colnames(mds)[2]<-"2007";colnames(mds)[3]<-"2008";colnames(mds)[4]<-"2009";
colnames(mds)[5]<-"2010";colnames(mds)[6]<-"2011";
ST2dist<-cbind(WeightedAs2006,WeightedAs2007,WeightedAs2008,WeightedAs2009,WeightedAs2010,WeightedAs2011)
#View(ST2dist)
ST2dist <-round(ST2dist[,],2)
ST2dist<-rbind(ST2dist,mds)
rownames(ST2dist)[16]<-"Mean difference (95% CI)"
#View(ST2dist)
ST2dist <- rbind(ST2dist, Level= c("County","County","County","County","County","County"))


write.xlsx(ST2dist,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 2.xlsx")

# Range and average number of CWSs serving a county in a year ?
finallong <- gather(finaldata, year, arsenic, Y2006:Y2011, factor_key=TRUE)
t06<-finallong[which(finallong$year=="Y2006"),]
t07<-finallong[which(finallong$year=="Y2007"),]
t08<-finallong[which(finallong$year=="Y2008"),]
t09<-finallong[which(finallong$year=="Y2009"),]
t10<-finallong[which(finallong$year=="Y2010"),]
t11<-finallong[which(finallong$year=="Y2011"),]

#Remove NA arsenic rows since those don't contribute 
t06<-t06[which(!is.na(t06$arsenic)),]
t07<-t07[which(!is.na(t07$arsenic)),]
t08<-t08[which(!is.na(t08$arsenic)),]
t09<-t09[which(!is.na(t09$arsenic)),]
t10<-t10[which(!is.na(t10$arsenic)),]
t11<-t11[which(!is.na(t11$arsenic)),]

library(data.table)
D <- data.table(t06)
counts <- D[, .(rowCount = .N), by = CountyFIPS]
counts
min(counts$rowCount) #
max(counts$rowCount) #
mean(counts$rowCount) #
g<-counts[which(counts$rowCount==152),] # Hillsborough, FL
...
####### 6. Supplemental Table 3. Distribution of 3-year averages for CWSs and counties ####### 
...
write.xlsx(Table1,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 3.xlsx")

### end
####### 7. Supplemental Table 4. Quantile regression results for CWS and county overall ####### 
...
write.xlsx(STable4,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 4.xlsx")
...
####### 8. Supplemental Table 5. Distribution of 3-year CWS averages, stratified by region  ####### 
...
write.xlsx(STable5,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 5.xlsx")

### end

####### 9. Supplemental Table 6. Quantile regression results stratified by region ######
set.seed(123)
...
write.xlsx(STable6,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 6.xlsx")

### end



####### 10. Supplemental Table 7. Distribution of 3-year county averages, stratified by region  ###### 
...
write.xlsx(STable7,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 7.xlsx")


### end


####### 11. Supplemental Table 8. Distribution of 3-year county averages, stratified by sociodemographic county cluster #######

table(county$cluster.number)
table(county$cluster.name)

## c1
c1<-county[which(county$cluster.number=="1"),];dim(c1)
c2<-county[which(county$cluster.number=="2"),];dim(c2)
c3<-county[which(county$cluster.number=="3"),];dim(c3)
c4<-county[which(county$cluster.number=="4"),];dim(c4)
c5<-county[which(county$cluster.number=="5"),];dim(c5)
c6<-county[which(county$cluster.number=="6"),];dim(c6)
c7<-county[which(county$cluster.number=="7"),];dim(c7)
c8<-county[which(county$cluster.number=="8"),];dim(c8)
c1long<-countylong[which(countylong$cluster.number=="1"),]
c2long<-countylong[which(countylong$cluster.number=="2"),]
c3long<-countylong[which(countylong$cluster.number=="3"),]
c4long<-countylong[which(countylong$cluster.number=="4"),]
c5long<-countylong[which(countylong$cluster.number=="5"),]
c6long<-countylong[which(countylong$cluster.number=="6"),]
c7long<-countylong[which(countylong$cluster.number=="7"),]
c8long<-countylong[which(countylong$cluster.number=="8"),]
...
# Distributions by 3-year period
# 2006-2008
nav<-nrow(c1[which(!is.na(c1$WeightedAs20062008)),])
nmiss<-nrow(c1[which(is.na(c1$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c1$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c1[which(c1$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c1[which(c1$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c1$WeightedAs20062008),na.rm=T))
mean<-mean(c1$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c1[which(!is.na(c1$WeightedAs20092011)),])
nmiss<-nrow(c1[which(is.na(c1$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c1$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c1[which(c1$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c1[which(c1$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c1$WeightedAs20092011),na.rm=T))
mean<-mean(c1$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
## c2
nav<-nrow(c2[which(!is.na(c2$WeightedAs20062008)),])
nmiss<-nrow(c2[which(is.na(c2$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c2$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c2[which(c2$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c2[which(c2$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c2$WeightedAs20062008),na.rm=T))
mean<-mean(c2$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c2[which(!is.na(c2$WeightedAs20092011)),])
nmiss<-nrow(c2[which(is.na(c2$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c2$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c2[which(c2$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c2[which(c2$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c2$WeightedAs20092011),na.rm=T))
mean<-mean(c2$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
## c3
#2006-2008
nav<-nrow(c3[which(!is.na(c3$WeightedAs20062008)),])
nmiss<-nrow(c3[which(is.na(c3$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c3$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c3[which(c3$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c3[which(c3$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c3$WeightedAs20062008),na.rm=T))
mean<-mean(c3$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c3[which(!is.na(c3$WeightedAs20092011)),])
nmiss<-nrow(c3[which(is.na(c3$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c3$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c3[which(c3$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c3[which(c3$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c3$WeightedAs20092011),na.rm=T))
mean<-mean(c3$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
## c4
#2006-2008
nav<-nrow(c4[which(!is.na(c4$WeightedAs20062008)),])
nmiss<-nrow(c4[which(is.na(c4$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c4$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c4[which(c4$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c4[which(c4$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c4$WeightedAs20062008),na.rm=T))
mean<-mean(c4$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c4[which(!is.na(c4$WeightedAs20092011)),])
nmiss<-nrow(c4[which(is.na(c4$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c4$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c4[which(c4$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c4[which(c4$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c4$WeightedAs20092011),na.rm=T))
mean<-mean(c4$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
## c5
#2006-2008
nav<-nrow(c5[which(!is.na(c5$WeightedAs20062008)),])
nmiss<-nrow(c5[which(is.na(c5$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c5$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c5[which(c5$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c5[which(c5$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c5$WeightedAs20062008),na.rm=T))
mean<-mean(c5$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c5[which(!is.na(c5$WeightedAs20092011)),])
nmiss<-nrow(c5[which(is.na(c5$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c5$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c5[which(c5$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c5[which(c5$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c5$WeightedAs20092011),na.rm=T))
mean<-mean(c5$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
## c6
#2006-2008
nav<-nrow(c6[which(!is.na(c6$WeightedAs20062008)),])
nmiss<-nrow(c6[which(is.na(c6$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c6$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c6[which(c6$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c6[which(c6$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c6$WeightedAs20062008),na.rm=T))
mean<-mean(c6$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c6[which(!is.na(c6$WeightedAs20092011)),])
nmiss<-nrow(c6[which(is.na(c6$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c6$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c6[which(c6$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c6[which(c6$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c6$WeightedAs20092011),na.rm=T))
mean<-mean(c6$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
## c7
nav<-nrow(c7[which(!is.na(c7$WeightedAs20062008)),])
nmiss<-nrow(c7[which(is.na(c7$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c7$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c7[which(c7$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c7[which(c7$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c7$WeightedAs20062008),na.rm=T))
mean<-mean(c7$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c7[which(!is.na(c7$WeightedAs20092011)),])
nmiss<-nrow(c7[which(is.na(c7$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c7$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c7[which(c7$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c7[which(c7$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c7$WeightedAs20092011),na.rm=T))
mean<-mean(c7$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
## c8
nav<-nrow(c8[which(!is.na(c8$WeightedAs20062008)),])
nmiss<-nrow(c8[which(is.na(c8$WeightedAs20062008)),])
quant<-as.data.frame(quantile(c8$WeightedAs20062008,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c8[which(c8$WeightedAs20062008<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c8[which(c8$WeightedAs20062008>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c8$WeightedAs20062008),na.rm=T))
mean<-mean(c8$WeightedAs20062008,na.rm=T)
colnames(quant)[1] <- "County 2006-2008"
Time1<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time1)[1] <- "N";rownames(Time1)[2] <- "N missing";rownames(Time1)[12] <- "% below MRL"
rownames(Time1)[13] <- "% above MCL";rownames(Time1)[14] <- "Geometric mean";rownames(Time1)[15] <- "Arithmetic mean"
# 2009-2011
nav<-nrow(c8[which(!is.na(c8$WeightedAs20092011)),])
nmiss<-nrow(c8[which(is.na(c8$WeightedAs20092011)),])
quant<-as.data.frame(quantile(c8$WeightedAs20092011,  probs = c(0,0.1, 0.25, 0.5, 0.75, 0.9,0.95, 0.99,1), na.rm=TRUE, scientific=F))
nmrl1<-nrow(c8[which(c8$WeightedAs20092011<1),])
nmrl<-round(100*(nmrl1/nav),1)
nmcl1<-nrow(c8[which(c8$WeightedAs20092011>=10),])
nmcl<-round(100*(nmcl1/nav),1)
gm<-exp(mean(log(c8$WeightedAs20092011),na.rm=T))
mean<-mean(c8$WeightedAs20092011,na.rm=T)
colnames(quant)[1] <- "County 2009-2011"
Time2<-rbind(nav,nmiss,quant,nmrl,nmcl,gm,mean)
rownames(Time2)[1] <- "N";rownames(Time2)[2] <- "N missing";rownames(Time2)[12] <- "% below MRL"
rownames(Time2)[13] <- "% above MCL";rownames(Time2)[14] <- "Geometric mean";rownames(Time2)[15] <- "Arithmetic mean"
...
STable8<-cbind(c1county,c2county,c3county,c4county,c5county,c6county,c7county,c8county)
write.xlsx(STable8,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 8.xlsx")


### end


####### 12. Supplemental Table 9. Distribution of 3-year CWS averages, stratified by county-cluster  ####### 
...
testtab<-STable9
testtab$`CWS 2006-2008`


write.xlsx(STable9,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 9.xlsx")
...
####### 13. Supplemental Table 10. Characteristics of counties by compliance categories using 10 ug/L cut-point ######
...
write.xlsx(STable10,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 10.xlsx")
...
####### 14. Supplemental Table 11. Characteristics of counties by compliance category using 5 ug/L cut-points #####
...
write.xlsx(STable11,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 11.xlsx")



### end

####### 15. Supplemental Table 12. Characteristics of counties by compliance categories using 1 ug/L cut-point ##### 
...
write.xlsx(STable12,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 12.xlsx")


### end


####### 16. Supplemental Tables 13 and 14: Counties in High/High and High/Low compliance categories ######
...
STable13<-hh[myvars]
write.xlsx(STable13,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 13.xlsx")
...
myvars<-c("CountyFIPS","Full.Name","PWSID","Adjusted.Total.Population.Served","ThreeYearAs20062008","ThreeYearAs20092011","Region")
STable14<-hl[myvars]
write.xlsx(STable14,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/output/Supplemental Table 14.xlsx")
...
####### 17. Supplemental Figure 3. Histograms of water arsenic concentrations at CWS and county level######
library(ggplot2)
library(ggpubr)
library(gridExtra)
library(gridExtra)
library(grid)
setwd("~/Google Drive/Research/EPAAsTrajectories/March2020")
folder.output <- "output"
if( !file.exists( folder.output ) ) {
  dir.create( file.path( folder.output ) )
}
### CWS level
pdf(file=paste0( folder.output , "/Supplemental Figure 3A.pdf"), 20, 10)
## CWS-level, all records, log-scale.
p1<-ggplot(finallong, aes(x=log(arsenic),  col=I("white"))) +
  geom_histogram(bins=40,fill="steelblue")+ ggtitle(expression(paste("Log-transformed"))) +
  labs(x = "Water arsenic concentration", y= "Count") +
  theme(plot.title = element_text(size=18, face="bold", hjust=0.5))+
  theme(strip.text.x = element_text(size = 18))+
  theme(plot.margin = margin(0, 0.5, 2, 0.5, "cm"))+
  theme(axis.text=element_text(size=18),
        axis.title=element_text(size=18))+
  scale_x_continuous(breaks = c(-1.049822,0,0.6931472,1.609438,2.302585,3.912023), labels = c("0.35", "1","2","5","10","50"),limits=c(-1.609438,3.912023))


## CWS-level, all records, orig scale.
p2<-ggplot(finallong, aes(x=arsenic,  col=I("white"))) +
  geom_histogram(bins=40,fill="steelblue")+ ggtitle(expression(paste("Original scale"))) +
  labs(x = "Water arsenic concentration", y= "Count") +
  theme(plot.title = element_text(size=18, face="bold", hjust=0.5))+
  theme(strip.text.x = element_text(size = 18))+
  theme(plot.margin = margin(0, 0.5, 2, 0.5, "cm"))+
  theme(axis.text=element_text(size=18),
        axis.title=element_text(size=18))+
  scale_x_continuous(breaks = c(0.35,5,10,50), labels = c("0.35","5","10","50"),limits=c(0.2,50))+
  scale_y_continuous(limits=c(0,6500))

text<-expression(paste("Distribution of water arsenic concentrations (",mu,"g/L) for all records (N=218,436)"))
tgrob <- text_grob(text, size=20)
grid.arrange(top = tgrob, 
             p1, p2, ncol=2)
dev.off()
part2<-grid.arrange(top = tgrob, 
                    p1, p2, ncol=2)


### County level
pdf(file=paste0( folder.output , "/Supplemental Figure 3B.pdf"), 20, 10)
## County-level, all records, log-scale.
p5<-ggplot(countylong, aes(x=log(arsenic),  col=I("white"))) +
  geom_histogram(bins=40,fill="steelblue")+ ggtitle(expression(paste("Log-transformed"))) +
  labs(x = "Water arsenic concentration", y= "Count") +
  theme(plot.title = element_text(size=18, face="bold", hjust=0.5))+
  theme(strip.text.x = element_text(size = 18))+
...
## County-level, all records, orig scale.
p6<-ggplot(countylong, aes(x=arsenic,  col=I("white"))) +
  geom_histogram(bins=40,fill="steelblue")+ ggtitle(expression(paste("Original scale"))) +
  labs(x = "Water arsenic concentration", y= "Count") +
  theme(plot.title = element_text(size=18, face="bold", hjust=0.5))+
  theme(strip.text.x = element_text(size = 18))+
...
text<-expression(paste("Distribution of water arsenic concentrations (",mu,"g/L) for all county records (N=16,238)"))
tgrob <- text_grob(text, size=20)
grid.arrange(top = tgrob, 
             p5, p6, ncol=2)
dev.off()

part1<-grid.arrange(top = tgrob, 
                    p5, p6, ncol=2)
## both together, final figure
pdf(file=paste0( folder.output , "/Supplemental Figure 3 Total.pdf"), 20, 20)
grid.arrange(part2, part1, ncol=1)
dev.off()

### end

####### 18. Figure 3: Raincloud plots by region: 2006-2008 ######
...
p1<-ggplot(data = pdata1, aes(y = log(ThreeYearAs20062008), x = fct_relevel(as.factor(Region),"Eastern Midwest" ,"Central Midwest", "Mid-Atlantic","Southeast"), fill = as.factor(Region))) +
  ...
  annotate(geom="text", x=0.5, y=3.2, label=expression(bold(paste("Maximum contaminant level"))),color="tomato4",size=5)+
  annotate("pointrange", x = 4.0, y = log(0.66), ymin = log(0.66), ymax = log(0.66), colour = "red", size = 1)+ # SE mean
  annotate("pointrange", x = 3.0, y = log(0.88), ymin = log(0.88), ymax = log(0.88), colour = "red", size = 1)+ # MA mean
  annotate("pointrange", x = 2.0, y = log(1.94), ymin = log(1.94), ymax = log(1.94), colour = "red", size = 1)+ # CMW mean
  annotate("pointrange", x = 1.0, y = log(2.03), ymin = log(2.03), ymax = log(2.03), colour = "red", size = 1.2)+ #EMW mean
  annotate(geom="text", x=4.5, y=1.3, label=expression(bold(paste("Mean: 0.66 (0.54, 0.79) µg/L"))),color="#525252",size=5)+ #SE 
  annotate(geom="text", x=3.5, y=1.3, label=expression(bold(paste("Mean 0.88 (0.73, 1.03) µg/L"))),color="#525252",size=5)+ # MA 
  annotate(geom="text", x=2.5, y=1.3, label=expression(bold(paste("Mean 1.94 (1.72, 2.15) µg/L"))),color="#525252",size=5)+ #  CMW
  annotate(geom="text", x=1.5, y=1.3, label=expression(bold(paste("Mean 2.03 (1.88, 2.18) µg/L"))),color="#525252",size=5)+ # EMW  
  annotate(geom="text", x=0.5, y=3.2, label=expression(bold(paste("Maximum contaminant level"))),color="tomato4",size=5)
...
p2<-ggplot(data = pdata2, aes(y = log(ThreeYearAs20062008), x = fct_relevel(as.factor(Region), "Southwest","Alaska/Hawaii","Pacific Northwest","New England"), fill = Region)) +
  ...
  annotate(geom="text", x=4.5, y=1.3, label=expression(bold(paste("Mean 2.05 (1.79, 2.30) µg/L"))),color="#525252",size=5)+ #NE
  annotate(geom="text", x=3.5, y=1.3, label=expression(bold(paste("Mean 2.15 (1.99, 2.31) µg/L"))),color="#525252",size=5)+ # PNW
  annotate(geom="text", x=2.5, y=1.3, label=expression(bold(paste("Mean 2.17 (1.67, 2.67) µg/L"))),color="#525252",size=5)+ # AKHI 
  annotate(geom="text", x=1.5, y=1.3, label=expression(bold(paste("Mean 3.59 (3.46, 3.72) µg/L"))),color="#525252",size=5)+ # SW 
  annotate(geom="text", x=0.5, y=3.2, label=expression(bold(paste("Maximum contaminant level"))),color="tomato4",size=5)+
  annotate("pointrange", x = 4, y = log(2.05), ymin = log(2.05), ymax = log(2.05), colour = "red", size = 1)+ # NE
  annotate("pointrange", x = 3, y = log(2.15), ymin = log(2.15), ymax = log(2.15), colour = "red", size = 1)+ # PNW
  annotate("pointrange", x = 2, y = log(2.17), ymin = log(2.17), ymax = log(2.17), colour = "red", size = 1)+ # AKHI
  annotate("pointrange", x = 1, y = log(3.59), ymin = log(3.59), ymax = log(3.59), colour = "red", size = 1) # SW 
fig1<-ggarrange(p1, p2, ncol=2, nrow=1, legend="none")

annotate_figure(fig1, top = text_grob("Average water arsenic concencentrations in CWSs, 2006-2008", color = "black", face = "bold", size = 20))

dev.off()

### end

####### 19. Figures 1, 4, and Supplemental Figure 4 - MAPS   #####
...
### Figure. 1 County-level estimated CWS water arsenic from 2006-2011 
# define color pal
...
myvars<-c("CountyFIPS","WeightedAs20062011" ,"Full.Name","group10")
...
g$Asgroup[g$group10==5]<-5 #missing both data periods
g$Asgroup[g$WeightedAs20062011<=1]<-1
g$Asgroup[g$WeightedAs20062011>1&g$WeightedAs20062011<=5]<-2
g$Asgroup[g$WeightedAs20062011>5&g$WeightedAs20062011<=10]<-3
g$Asgroup[g$WeightedAs20062011>10]<-4
table(g$Asgroup)

state_map <- map_data("state")
pdf(file=paste0( folder.output , "/Fig 2. CWS As 20062011.pdf"), 15, 9)
mall<- ggplot() + geom_polygon(data = g, aes(x=long, y = lat, group = group, fill=as.factor(Asgroup)), color = "antiquewhite4") + 
  coord_fixed(1.3)+ scale_fill_manual(values = mypal,  na.value = "grey90", labels=c("<=1",">1 - 5",
                                                                                     ">5 - 10",">10", "Inadequte data",
                                                                                     "No data available")) +
  
  geom_path( data = state_map, aes(x=long,y=lat, group=group), colour = "black",size=0.5)+
  labs(title="County-level weighted average CWS arsenic concentrations, 2006-2011", y="Latitude", x="Longitude")+
  theme(plot.title = element_text(face="bold",size=20))+
  theme(legend.position = c(.9, .2))+
  labs(fill=expression(paste("Concentration (", mu, "g/L)")))
mall
dev.off()
mall


## Supplemental Figure 4. Three-year averages at county-level, 2006-2008
# color pal:
supfigpal <- c("#C3FA9F","#F6F1A9","#FDC030","#F08080", "#B7B3B3")

ff<-county
mv<-c("CountyFIPS","Flag20062008","Flag20092011")

myvars<-c("CountyFIPS","WeightedAs20062008" ,"Full.Name","Flag20062008","Flag20092011")
mydata<-ff[myvars]
colnames(mydata)[colnames(mydata)=="Full.Name"] <- "subregion"
...
describe(g$group)#0 missing; 87949 , 3085 unique
describe(g$CountyFIPS)#0 missing 
describe(g$WeightedAs20062008)#n=68679; 19270 missing

#Create categorical weighted arsenic variable to be plotted
g$Asgroup<-NA
g$Asgroup[g$WeightedAs20062008<=1]<-1
g$Asgroup[g$WeightedAs20062008>1&g$WeightedAs20062008<=5]<-2
g$Asgroup[g$WeightedAs20062008>5&g$WeightedAs20062008<=10]<-3
g$Asgroup[g$WeightedAs20062008>10]<-4
g$Asgroup[g$Flag20062008==1]<-5
table(g$Asgroup)

state_map <- map_data("state")
pdf(file=paste0( folder.output , "/Supplemental Figure 4A - CWS As 20062008.pdf"), 15, 9)
ma0608<- ggplot() + geom_polygon(data = g, aes(x=long, y = lat, group = group, fill=as.factor(Asgroup)), color = "antiquewhite4") + 
  coord_fixed(1.3)+ scale_fill_manual(values = supfigpal,  na.value = "grey90", labels=c("<=1",">1 - 5",
                                                                                         ">5 - 10",">10", "Inadequate data", "No data available")) +
  
  geom_path( data = state_map, aes(x=long,y=lat, group=group), colour = "black",size=0.5)+
  labs(title="County-level weighted three-year average CWS arsenic concentrations, 2006-2008", y="Latitude", x="Longitude")+
  theme(plot.title = element_text(face="bold",size=20))+
  theme(legend.position = c(.9, .2))+
  labs(fill=expression(paste("Concentration (", mu, "g/L)"))) 
ma0608
...
## Supplemental Figure 4B. Three-year averages at county-level, 2009-2011

ff<-county
myvars<-c("CountyFIPS","WeightedAs20092011" ,"Full.Name","Flag20062008","Flag20092011")
mydata<-ff[myvars]
colnames(mydata)[colnames(mydata)=="Full.Name"] <- "subregion"
describe(mydata$subregion)
setwd("~/Google Drive/Research/County level exposure and outcomes/Maps")
...
j$CountyFIPS[j$subregion=="norfolk"]<-51710
mydata$subregion<-NULL
describe(mydata$WeightedAs20092011)
...
describe(g$group)#0 missing; 87949 , 3085 unique
describe(g$CountyFIPS)#0 missing 
describe(g$WeightedAs20092011)#n=68679; 19270 missing

#Create categorical weighted arsenic variable to be plotted
g$Asgroup<-NA
g$Asgroup[g$WeightedAs20092011<=1]<-1
g$Asgroup[g$WeightedAs20092011>1&g$WeightedAs20092011<=5]<-2
g$Asgroup[g$WeightedAs20092011>5&g$WeightedAs20092011<=10]<-3
g$Asgroup[g$WeightedAs20092011>10]<-4
g$Asgroup[g$Flag20092011==1]<-5
table(g$Asgroup)

state_map <- map_data("state")
pdf(file=paste0( folder.output , "/Supplemental Figure 4B - CWS As 20092011.pdf"), 15, 9)
ma0911<- ggplot() + geom_polygon(data = g, aes(x=long, y = lat, group = group, fill=as.factor(Asgroup)), color = "antiquewhite4") + 
  coord_fixed(1.3)+ scale_fill_manual(values = supfigpal,  na.value = "grey90", labels=c("<=1",">1 - 5",
                                                                                         ">5 - 10",">10", "Inadequate data", "No data available")) +
  
  geom_path( data = state_map, aes(x=long,y=lat, group=group), colour = "black",size=0.5)+
  labs(title="County-level weighted three-year average CWS arsenic concentrations, 2009-2011", y="Latitude", x="Longitude")+
  theme(plot.title = element_text(face="bold",size=20))+
  theme(legend.position = c(.9, .2))+
  labs(fill=expression(paste("Concentration ( ", mu, "g/L )"))) 
ma0911
...
## Figure 2 - Map of county-level compliance categories
ff<-county
county$group10[which(is.na(county$group10))]<-10
table(county$group10)

myvars<-c("CountyFIPS","WeightedAs20092011" ,"Full.Name","group10")
mydata<-ff[myvars]
colnames(mydata)[colnames(mydata)=="Full.Name"] <- "subregion"
describe(mydata$subregion)
setwd("~/Google Drive/Research/County level exposure and outcomes/Maps")
...
mypal <- c("#C3FA9F","#FDC030", "#F08080",  "#8B0000","#B7B3B3")
# we need in this order: low/low, high/low, low/high, high/high
# C3FA9F = low-low; light green
# F6F1A9= high low (orange) 
# F08080 = low/high (purple)
# 8B0000 = high/high dark red
# B7B3B3 = dark grety (missing one data period

state_map <- map_data("state")
pdf(file=paste0( folder.output , "/Figure 3 - Compliance category map.pdf"), 15, 9)
traj<- ggplot() + geom_polygon(data = g, aes(x=long, y = lat, group = group, fill=factor(group10)), color = "antiquewhite4") + 
  coord_fixed(1.3)+ scale_fill_manual(values = mypal,  na.value = "grey95", labels=c("Low/Low","Low/High",
                                                                                     "High/Low","High/High",
                                                                                     "Missing one data period", "No data available"))+
  geom_path( data = state_map, aes(x=long,y=lat, group=group), colour = "black",size=0.5)+
  labs(title=expression(bold(paste("County compliance category membership using 10 ", mu, "g/L cut-points"))), y="Latitude", x="Longitude")+
  theme(legend.position = c(.9, .2))+
...
####### 20. Figure 2: Quantile regression results stratified by region ######
...
pdf(file=paste0( folder.output , "/Figure 3 quantreg overall.pdf"), 12, 8)
...
ggplot(QuantFig, aes(Quantile, group=1)) + 
  ...
  geom_ribbon(aes(ymin=LL, ymax=UL), alpha=0.2,color=blue_s)+
  facet_wrap(~Region)
...
pdf(file=paste0( folder.output , "/C. Figure 2. QR plots by region, order by 20062008 mean.pdf"), 15, 9)
ggplot(QuantFig2[which(QuantFig2$Region!="Alaska/Hawaii"),], aes(Quantile, group=1,fill=Region)) + 
  geom_line(aes(y=Beta),color="#525252")+ 
  ...
  geom_text(data=ann_text, mapping=aes(x=x, y=y, label=label,color="#525252",fontface = "bold", size=10))
dev.off()
