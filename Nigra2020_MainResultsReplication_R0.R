...
# Count number of records for each CWS
tocount<-As6YR3
tocount$YearlyDate<-format(tocount$Sample.Collection.Date, format="%Y")
tocount$YearlyDate <- substr(tocount$YearlyDate, 0, 4) #only keep first 10characters in character vector (keep date, not time)
describe(tocount$YearlyDate)
table(tocount$YearlyDate)
tocount1<-tocount[which(tocount$YearlyDate=="2006"| tocount$YearlyDate=="2007"|tocount$YearlyDate=="2008"),]

counting<-as.data.frame(table(tocount1$PWSID))
range(counting$Freq) #1, 791
mean(counting$Freq) # 3

tocount2<-tocount[which(tocount$YearlyDate=="2009"| tocount$YearlyDate=="2010"|tocount$YearlyDate=="2011"),]
counting<-as.data.frame(table(tocount2$PWSID))
range(counting$Freq) #1, 1506
mean(counting$Freq) # 4
...
#######       1E. Table 1 addition. N records by CWS characteristic (1000s) ######
...
# By county-cluster, use:
fdat2<-merge(dat,finaldatadups3,by="PWSID",all=T)
fdat0911<-fdat2[which(!is.na(fdat2$ThreeYearAs20092011)),];dim(fdat0911)
...
######        1F. Table SX. Mean difference over time by subgroup, adjusting for source water type and size served ######
cws$ThreeYearAs20062008<-as.numeric(cws$ThreeYearAs20062008)
cws$ThreeYearAs20092011<-as.numeric(cws$ThreeYearAs20092011)
...
TableSX<-cbind(allcws,gwcws,swcws,q1cws,q2cws,q3cws,q4cws,q5cws,AKHIcws,CMWcws,EMWcws,MAcws,NEcws,PNWcws,SEcws,SWcws,c1cws,c2cws,c3cws,c4cws,c5cws,c6cws,c7cws,c8cws)

# Transpose into better format
TableSX1 <- data.frame(t(TableSX[]))
colnames(TableSX1) <- TableSX[, 1]
colnames(TableSX1)[1]<-"N";colnames(TableSX1)[2]<-"% at/above MCL";colnames(TableSX1)[3]<-"Arithmetic mean";colnames(TableSX1)[4]<-"Mean difference";colnames(TableSX1)[5]<-"Percent difference"



write.xlsx(TableSX1,row.names=T, file="~/Google Drive/Research/EPAAsTrajectories/March2020/EHP resubmissoin/July 2020 R0 EHP/output/TableSX1 .xlsx")
...
# Fischer exact test:
fisher.test(cws$amcl1,cws$amcl2, conf.level = 0.95)
fisher.test(cws$amcl1,cws$amcl2, conf.level = 0.95)$conf.int
testor = rbind(c(35411,995), + c(35668,738) ); testor
fisher.test(testor, conf.level = 0.95)
